...
<div id="vm-8">
	<div v-if="formKey == 'list'">
		<table width="100%" class="tb02 tc">
			...
			<tbody>
				<tr v-for="(o,i) in listObj.lectureBoardList">
					...
					<td class="tl"><a href="#none" :onclick="'call8ViewForm('+o.idx+')'">{{o.title}}
					<span v-if="o.commentCnt>0">(<strong class="fc_red">{{o.commentCnt }}</strong>)</span>
					</a></td>
					<td>{{o.regNm}}</td>
		...
		<div class="fr tc">
			<button onclick="call8RegForm()" class="btn01 btn_greenl">등록</button>
		</div>
		<!--// paging //-->
					<div class="page">
						<div class="inner cf">
							...
							<div class="page_prev0">	
								<a href="javascript:;" :onclick="'fn8_pageTab('+1+');'">&lt; 처음</a>
							</div>
							<div class="page_prev0">
								<a href="javascript:;" :onclick="'fn8_pageTab('+(listObj.pageNavi.currentPageNo - 1)+');'">&lt; 이전</a>
							</div>
							</template>
							...
							<div class="page_nomal">
								<a href="javascript:;" :onclick="'fn8_pageTab('+o+');'">{{o}}</a>
							</div>
							</template> </template>
							...
							<div class="page_next0">
								<a href="javascript:;" :onclick="'fn8_pageTab('+(listObj.pageNavi.currentPageNo + 1) + ');'">다음 &gt;</a>
							</div>
							<div class="page_next0">
								<a href="javascript:;" :onclick="'fn8_pageTab('+(listObj.pageNavi.lastPageNo ) + ');'">끝 &gt;</a>
							</div>
							</template>
						</div>
					</div>
	...
	<div v-if="formKey == 'reg'">
		...
		<table class="w100 tb02 tl">
			<tbody>
				...
				<tr>
					<th class="tc">제목</th>
					<td><input type="text" v-model="infoObj.title" id="title-8" class="ip4 tl"/></td>
				</tr>
				<tr>
					<th class="tc">내용</th>
					<td>
					<comp8-ckeditor v-model="infoObj.content"></comp8-ckeditor>
					</td>
				</tr>
				<tr>
					...
					<td class="tl" colspan="">
						<!-- dropzone html -->
						<comp8-dropzone ></comp8-dropzone>
						<div id="area-filelist">
						<ul>
							<li v-for="o in fileList">{{o.fileOrg }}, <a href="#none" :onclick="'fn8_delFile('+o.fileSeq+')'">[삭제]</a></li>
						</ul>
						</div>
					</td>
				</tr>
		...
		<div class="fr tc">
		<button onclick="save8InfoObj()" class="btn01 btn_greenl">저장</button>
		<button onclick="call8EduBbsListData()" class="btn01 btn_grayl">취소</button>
		</div>
	...
	<div v-if="formKey == 'view'">
		<div class="tstyle_view">
			...
			<div class="w100 cf mgt20 mgb20">
				<a onclick="call8EduBbsListData()" class="fl btn02 btn_gray">목록</a> 
				<a v-if="'${sessionScope.sessionUserInfo.userId }'==infoObj.regId" :onclick="'call8RegForm('+infoObj.idx+')'" href="javascript:;" class="fr btn02 btn_blue">수정</a> 
				<a :onclick="'fn8_delEduBbs('+infoObj.idx+')'" href="javascript:;" class="fr btn02 btn_orange mgr5">삭제</a>
			</div>
			...
			<div class="commentWrap">
				...
				<div class="commentWrite">
					<div class="box_textarea">
						<textarea placeholder="최대 500자 이내로 입력하세요." id="comment8-0" maxlength="600" class="comtTa" onkeyup="fn_checkTextArea(this)"></textarea>
...
Vue.component('comp8-ckeditor', {	
	template: '<textarea v-ckeditor id="ckeditCn8" v-model="value"></textarea>',
	props:['value'],
	directives : {
        ckeditor : {
            inserted:function (el, binding, vNode) {
				...
				CKEDITOR.replace('ckeditCn8', {
					filebrowserUploadUrl : '${utcp.ctxPath}/editot/popupUpload.json?gubun=lectureBoard&prefixStr=eduCommunity_'+$('#eduSeq').val(),
					height : 400,
				});
            }
        }
...
Vue.component('comp8-dropzone', {	
	template: '<div id="area8-dropzone" class="dropzone" v-dropzone> <div class="fallback"> <input name="file" type="file" multiple /> </div> <input name="image_url" id="image_url" type="hidden" /></div>',
	props:['value'],
	directives : {
        dropzone : {
            inserted:function (el, binding, vNode) {
            	if (typeof myDropzone8 !== "undefined"){
					myDropzone8.destroy();
				}
				console.log(2);
				Dropzone.autoDiscover = false;
				myDropzone8 = new Dropzone("#area8-dropzone", {
					url : "${utcp.ctxPath}/admin/edu/bbs/dropzoneUpload.ajax",
					paramName: "file",
			        maxFilesize: 5,
			        maxFiles: 5,
			        acceptedFiles: ".pdf,.hwp,.xlxs",
			        ...
			        init: function () {
			        	this.on("success", function(file, returnedData, myEvent) {
			        		...
			        		call8ViewForm(vm_8.infoObj.idx);
			           	}); 
			        	this.on('sending', function(file, xhr, formData) {
			        		if(formData.get('idx')==null){
			        			
			        			formData.append('idx', vm_8.infoObj.idx);
			        			formData.append('eduSeq', $('#eduSeq').val());
			        		}
			           	});  
			        	this.on("addedfile", function(file) { 
			        		addCheckFile(myDropzone8,file);
			        	});
			        }
				});
            }
	...
	*/
	...
	function save8InfoObj(){
		var eduSeq = $('#eduSeq').val();
		
		var idx = vm_8.infoObj.idx;
		var title = vm_8.infoObj.title;
		console.log(1,title);
		if(title == '' || title == null){
			$('#title-8').focus();
			alert('제목은 필수입력입니다.');
		...
		//var content = vm_8.infoObj.content;
		var content;
		if (CKEDITOR.instances.ckeditCn8.getData() == "") {
			alert('내용은 필수입력입니다.');
			 CKEDITOR.instances.ckeditCn8.focus();
			return false;
		} else {
			content = CKEDITOR.instances.ckeditCn8.getData();
		}	
		
		$.ajax({
			data:{eduSeq : eduSeq, boardType : 'eduCommunity', idx : idx, title : title, content : content },
			url:'${utcp.ctxPath}/admin/edu/bbs/saveInfoObj.ajax',
			success:function(r){
				if(r.result == 1){
					
					vm_8.infoObj.idx = r.data.idx;
					if(myDropzone8.getQueuedFiles().length > 0){
						homeLoader.show();
	                    myDropzone8.processQueue();//excute upload!!
	                }else{
						call8ViewForm(r.data.idx);
	                }
	...
	function call8RegForm(idx){
		...
		$.ajax({
			...
			success:function(r){
				if(r.result == 1){
					vm_8.infoObj = r.data.lectureBoardInfo;
					vm_8.fileList = r.data.attachList;
					console.log(1);
					vm_8.formKey = 'reg';
		...
		$.ajax({
			...
			success:function(r){
				if(r.result == 1){
					vm_8.infoObj = r.data.lectureBoardInfo;
					vm_8.fileList = r.data.attachList;
					vm_8.cmntList = r.data.cmntList;
					vm_8.formKey = 'view';
					$('.commentWrite textarea[id^=comment8-0]').val('');
				}
			}
		});
	}
	function fn8_pageTab(pageNo){
		call8EduBbsListData(pageNo);
	}
	function fn8_delFile(fileSeq){
		...
		$.ajax({
			...
			success:function(r){
				if(r.resultCode==1){
					call8RegForm(vm_8.infoObj.idx);
				}
			}
		});
	}
	function fn8_delEduBbs(idx){
