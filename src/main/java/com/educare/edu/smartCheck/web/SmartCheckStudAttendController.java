...
@Controller
@RequestMapping(value="/smartCheck/stud/attend/")
public class SmartCheckStudAttendController {
	
	/** org.slf4j.Logger */
	private static final Logger LOG = LoggerFactory.getLogger(SmartCheckStudAttendController.class);
	
	@Resource(name="SmartcheckService")
	private SmartcheckService smartCheckService;
	
	
	@RequestMapping(value = "checkAuthNo.json", method = {RequestMethod.POST})
    public String authNoClose(HttpSession httpSession, ModelMap model
            ,SmartCheckVO params
            ) {
    	int result = smartCheckService.checkAuthNo(params);
