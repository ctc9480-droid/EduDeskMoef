...
<body>
	<div id="wrap">
		<div class="certi8 entercerti" id="pdfDiv">
			...
			<ul class="tl">
				<li><div class="certiNameTit">소 속</div><div class="pdl3 pdr3">:</div><div class="certiName">${certMap.user.userOrgNm }</div></li>
				<li><div class="certiNameTit">성명</div><div class="pdl3 pdr3">:</div><div class="certiName">${certMap.user.userNm} (${utcp.convDateToStr(utcp.convStrToDate(certMap.user.birth,'yyyyMMdd'),'yyyy-MM-dd')})</div></li>
				<li><div class="certiNameTit">과정</div><div class="pdl3 pdr3">:</div><div class="certiName">${certMap.lctre.ctg1Nm }</div></li>
				<li><div class="certiNameTit">교육기간(시간)</div><div class="pdl3 pdr3">:</div><div class="certiName">
			${utcp.convDateToStr(utcp.convStrToDate(certMap.lctre.eduPeriodBegin,'yyyy-MM-dd'),'yyyy.MM.dd')}
				~ ${utcp.convDateToStr(utcp.convStrToDate(certMap.lctre.eduPeriodEnd,'yyyy-MM-dd'),'yyyy.MM.dd')}
			(${certMap.classTime }) </div></li>
			</ul>
			
		 
			<p class="entertxt tl txt1">
			위 사람은 기획재정부에서 위와 같이 교육과정을 수료하였으므로 이 증서를 수여합니다.
			</p>
			...
			<p class="date enterdate">
			<span class="ctYear">${utcp.convDateToStr(certMap.stdnt.certDe,'yyyy') }</span>년
			<span class="ctMonth">${utcp.convDateToStr(certMap.stdnt.certDe,'MM') }</span>월
			<span class="ctDate">${utcp.convDateToStr(certMap.stdnt.certDe,'dd') }</span>일
			</p>
