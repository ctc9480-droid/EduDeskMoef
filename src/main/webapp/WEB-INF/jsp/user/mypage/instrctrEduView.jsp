...
<div class="listWrap" style="padding-top: 0" id="vue-instrctrEduDetail">
	<div id="viewDiv">
		<div class="box_list" style="margin-bottom: 0px;">
			<ul>
				<li class="flex" style="border-bottom: 0px;">
					<span class="img_logo"> <c:choose>
							<c:when test="${lctre.imgUseYn eq 'Y' && not empty lctre.imgRename }">
								<img src="<spring:eval expression="@prop['cloud.cdn.url']"/>/upload/web/lctreThum/${lctre.imgRename }" alt="${lctre.eduNm }" width="112px" height="149px">
							</c:when>
							<c:otherwise>
					...
					<div class="text_title">
						<div>
							<p style="margin-top: 0;" class="mgb5"><span class="mgr3">[${lctre.lctreTypeNm}]</span> ${lctre.eduCtgryNm } ${lctre.addStatusBox }</p> <strong>${lctre.eduNm }</strong>
					...
					<div class="text_info">
						<div>
							...
							<br />
							<span> [교육] <b>${lctre.eduPeriodBegin}</b>
							</span>
							<br />
							<span class="fw_500">${lctre.personnel}명 모집 (<strong class="color fw_500">${lctre.rceptCnt}</strong>명 신청)
							</span>
						</div>
					</div>
				</li>
		...
		<div class="board_tab_onoff">
			...
			<ul class="board_tab">
				<li>
					<p><a href="#none">내용 상세보기</a></p>
				</li>
				<li>
					<p><a href="#none">교육 시간표</a></p>
				</li>
				<li>
					<p><a href="#none">수강생 현황</a></p>
				</li>
			</ul>
			<!--// board_tab //-->
			...
			<div class="board_tab_con">
				...
				<div class="cont">
							...
							<tr>
								...
								<td><c:choose>
										<c:when test="${lctre.fee > 0 }">
											<fmt:formatNumber value="${lctre.fee }" maxFractionDigits="3" /> 원
							<c:if test="${lctre.groupFee > 0 }">
								/ 단체 <fmt:formatNumber value="${lctre.groupFee }" maxFractionDigits="3" /> 원
							</c:if>
										</c:when>
										<c:otherwise>
							무료
					...
					<div class="tb_btn">
						<ul>
							<a href="${utcp.ctxPath}/user/mypage/instrctrEdu${gubun }List.do"><li class="left">목록</li></a>
						</ul>
					</div>
					...
					<table class="tc w100 timetb tb02">
						...
						<tbody>
							...
							<tr>
								...
								<td>${o.classHowNm }</td>
								<td>
								<c:if test="${o.classHow ne 0 }">
								자동출석
								</c:if>
								<c:if test="${o.classHow eq 0 }">
								<a href="#none" onclick="fn_openCheckList(${o.eduSeq})" class="btn04 btn_orange">출석부</a>
								</c:if>
								</td>
							</tr>
							</c:forEach>
					...
					<table width="100%" class="tb02 tc">
						...
						<tbody>
							<tr v-for="(vo,i) in tab1.dataList" class="score_data">
								...
								<td><input name="workshopScore" :id="'tab1-workshopScore-'+i" type="text" class="ip5 tc" placeholder="40" v-model="vo.workshopScore" maxlength="3" numberonly style="width: 50px;" /></td>
								<td><input name="examScore" :id="'tab1-examScore-'+i" type="text" class="ip5 tc" placeholder="40" v-model="vo.examScore" maxlength="3" numberonly style="width: 50px;" /></td>
								<td class="fc_red">
								<c:choose>
...
<script type="text/javascript">
	$(function() {
		...
		$('.board_tab_onoff>.board_tab_con> div:first-child').css('display',
				'block');
		$('.board_tab_onoff>.board_tab li:first-child').addClass('active');
		$('.board_tab_onoff').delegate(
				'.board_tab>li',
				'click',
				function() {
					...
					$(this).parent().next('.board_tab_con').children().hide()
							.eq(index).show();
	...
	function fn_gooroomee() {
		$.ajax({
			...
			url : '${utcp.ctxPath}/user/remote/gooroomee/login.json',
