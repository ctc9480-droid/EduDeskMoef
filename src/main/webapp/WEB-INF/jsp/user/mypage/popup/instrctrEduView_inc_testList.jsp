...
<div class="remodal" data-remodal-id="md-tab10-testReg" id="vm-tab10-testReg" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc" data-remodal-options="closeOnOutsideClick: false">
	...
	<div class="modal-content">
			...
			<div class="tbBox1 mgt0">
				<table class="tl w100 tb04">
					...
					<tbody>
						...
						<tr>
							<th class="">사용여부</th>
							<td class="tl"><input type="radio" name="edu_10_reg_status" v-model="infoObj.status" value="1" id="edu-10-reg-status1"> <label class="mgr10" for="edu-10-reg-status1"> 사용 </label> <input type="radio" name="edu_10_reg_status" v-model="infoObj.status" value="2" id="edu-10-reg-status2"> <label class="mgr10" for="edu-10-reg-status2"> 사용안함 </label></td>
						</tr>
						<tr>
							<th class="">과목</th>
							<td class="tl">
						...
						<tr>
							<th>시험시간</th>
							<td class="tl"><comp-datepicker v-model="infoObj.startDt"></comp-datepicker> <select v-model="infoObj.startHh">
							...
							</select> ~ <comp-datepicker v-model="infoObj.endDt"></comp-datepicker> <select v-model="infoObj.endHh">
									<option v-for="o in 23" :value="o < 10 ? '0' + o : o">{{o < 10 ? '0' + o : o}}</option>
		...
		<div class="modal-footer">
			...
			<button @click="saveInfoObj(2)" class="remodal-confirm btn02 btn_green" v-if="infoObj.tryNo != 2 && infoObj.testSeq > 1 && infoObj.test2Cnt == 0">재평가 등록</button>
			<button @click="saveInfoObj" class="remodal-confirm btn02 btn_green">등록</button>
		</div>
	</div>
</div>
<script>
var vm_tab10_infoObj = {testSeq: 0,status: 1,startHh:'09',startMm:'00',endHh:'17',endMm:'00',tryNo: 1,prntTestSeq: 0};
...
<%-- 
 ...
 --%>
	<table class="tb07 w100">
		...
		<tbody>
			<tr v-for="(o,i) in stdntList">
				...
				<td>{{o.marks}}/100</td>
				<td>{{o.addGradeNm}}</td>
				<td><a href="javascript:;" @click="openStdntResult(o)" class="btn04 btn_orange testResultBtn">결과보기</a></td>
			</tr>
		</tbody>
	</table>
	<div class="fl tc">
	...
	<div class="fr tc">
		...
		<button @click="openResultAll('N')" class="btn02 btn_gray" >결과닫기</button>
		</template>
	</div>
</div>
<script>
			...
			$.ajax({
				...
				success: function(r){
					if(r.result == 1){
						...
						_this.test.status = r.data.test.status;
					}
				}
			});
		},
