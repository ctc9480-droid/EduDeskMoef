...
<%
	String device = LncUtil.getDevice(request);
	String mobileInfo = "";
	if ("M".equals(device)) {
		mobileInfo = "Mobile";
	}
	...
	//String customize 	= "";		
	String customize 	= mobileInfo;	// 없으면 기본 웹페이지 / Mobile : 모바일페이지
	
    // CheckPlus(본인인증) 처리 후, 결과 데이타를 리턴 받기위해 다음예제와 같이 http부터 입력합니다.
	//리턴url은 인증 전 인증페이지를 호출하기 전 url과 동일해야 합니다. ex) 인증 전 url : http://www.~ 리턴 url : http://www.~
	//String sReturnUrl = "http://localhost:8080/user/checkplus_success.do";      // 성공시 이동될 URL
    //String sErrorUrl = "http://localhost:8080/user/checkplus_fail.do";          // 실패시 이동될 URL
    String sReturnUrl = domainNm + "/user/checkplus_success.do?onlyMobile="+request.getParameter("onlyMobile");      // 성공시 이동될 URL
    String sErrorUrl = domainNm + "/user/checkplus_fail.do";          // 실패시 이동될 URL
