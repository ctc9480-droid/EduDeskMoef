...
<title><spring:eval expression="@prop['service.name']"/></title>
	...
	<meta name="keywords" content="<spring:eval expression="@prop['service.name']"/>, 학습관리, 시스템, 교육, 단체, 개인">
...
<link rel="stylesheet" type="text/css" href="${utcp.ctxPath}/resources/user/css/main.css?2">
<link rel="stylesheet" type="text/css" href="${utcp.ctxPath}/resources/user/css/sub.css?1">
<link rel="stylesheet" type="text/css" href="${utcp.ctxPath}/resources/user/css/slick.css?1" />
<link rel="stylesheet" type="text/css" href="${utcp.ctxPath}/resources/user/css/common.css?1">
...
<script type="text/javascript" src="${utcp.ctxPath}/resources/common/js/util.js?1"></script>
	...
	<div id="wrap">
		...
		<div id="container">
			...
			<div id="content">
				...
				%>
				<c:if test="${fn:indexOf(baseLayout_uri,'/mypage/')>=0  && fn:indexOf(baseLayout_uri,'/updUser.do') < 0 }">
				...
				<c:when test="${not empty menuList}">
					<ul>
						<c:forEach items="${menuList }" var="o">
							<c:if test="${o.st == 1 }">
							<c:choose>
							<c:when test="${fn:indexOf('updUser,updPw,familyMember',o.cd)>=0 }">
								<li class="upd fr">
									<a href="${utcp.ctxPath}${o.url }" class="${not empty o.on?'on':'' }"><i class="fas ${o.cd eq 'updUser'?'fa-user-edit':'fa-unlock' }"></i>${o.nm }</a>
								</li>
							</c:when>
							<c:otherwise>
								<li class="${fn:indexOf(o.url,'/mypage/')>=0?'fl':'' }">
									<a href="${utcp.ctxPath}${o.url }" class="${not empty o.on?'on':'' }">${o.nm }</a>
								</li>
							</c:otherwise>
							</c:choose>
							</c:if>
