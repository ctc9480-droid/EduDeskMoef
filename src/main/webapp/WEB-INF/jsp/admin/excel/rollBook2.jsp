	...
	<table class="tc w100">
		<thead bgcolor="#f7f8fa">
			<tr>
				...
				<th colspan="3">${utcp.convDateToStr(utcp.convStrToDate(o.eduDt,'yyyyMMdd'),'MM.dd (E)')}</th>
				</c:forEach>
<!-- 				<th rowspan="2">합계</th> -->
			</tr>
			<tr>
				<c:forEach items="${ dateList}" var="o">
				<th>아침<br/>
				<c:if test="${ o.isQr1 == 1}">${utcp.convDateToStr(o.qr1BeginDe,'HH:mm')} ~ ${utcp.convDateToStr(o.qr1EndDe,'HH:mm')}</c:if>
				</th>
				<th>점심<br/>
				<c:if test="${ o.isQr2 == 1}">${utcp.convDateToStr(o.qr2BeginDe,'HH:mm')} ~ ${utcp.convDateToStr(o.qr2EndDe,'HH:mm')}</c:if>
				</th>
				<th>저녁<br/>
				<c:if test="${ o.isQr3 == 1}">${utcp.convDateToStr(o.qr3BeginDe,'HH:mm')} ~ ${utcp.convDateToStr(o.qr3EndDe,'HH:mm')}</c:if>
				</th>
				</c:forEach>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${ stdntList}" var="o" varStatus="i">
			<tr>
				<td>${i.count}</td>
				<td>${o.userNm}</td>
				<td>${o.userOrgNm}</td>
				...
				<td style="text-align:center;">${o2.at1Cd}<br/>
				${o2.at1HhMm}</td>
				<td style="text-align:center;">${o2.at2Cd}<br/>
				${o2.at2HhMm}</td>
				<td style="text-align:center;">${o2.at3Cd}<br/>
				${o2.at3HhMm}</td>
				</c:forEach>
