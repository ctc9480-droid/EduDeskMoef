...
<table class="tb02 mt30">
	...
	<c:forEach items="${lectureCommunityList }" var="list">
		<tr>
			<td>${(pageNavi.totalRecordCount+1)-list.rNum }</td>
			<td class="tl"><a href="javascript:fn_communityView('${list.idx }');">
			<c:if test="${list.scrtyYn eq 'Y' }">
			<img src="${utcp.ctxPath}/resources/user/image/icon/icon_online.png" alt="자물쇠 아이콘" />
			</c:if>
			<c:choose>
			<c:when test="${!list.myReg and list.scrtyYn eq 'Y' }">
			비밀글 입니다. 
			</c:when>
			<c:otherwise>
			${list.title }
			...
			<!-- 
			<i class="fa-solid fa-n fc_white bg_red pd3 fs_12 br3"></i>
			 -->
			</a></td>
			<td>${list.addRegNmMasked }</td>
	...
	<c:if test="${empty lectureCommunityList }">
		<tr>
	...
	<div class="board_pager" style="margin-bottom:40px;">
		...
		<div class="w100 tr">
                        <a href="javascript:fn_communityReg();" class="btn02 btn_blue">작성</a>
        </div>
	</div>
<script>
	function fn_communityView(idx){
		$('#tabAction').val('communityView');
		$('#boardIdx').val(idx);
		$('#myEduForm').submit();
	}
	function fn_communityReg(){
		$('#boardIdx').val(0);
