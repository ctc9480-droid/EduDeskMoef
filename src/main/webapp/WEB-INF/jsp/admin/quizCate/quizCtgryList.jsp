...
function fn_upd(qstnCtgSeq, depth){
	//console.log("fn_upd called - qstnCtgSeq", qstnCtgSeq);
	//alert("fn_upd called - qstnCtgSeq", qstnCtgSeq);
	//if(depth==3){
	//	$('#area-tr-passCd').show();
	//}else{
	//	$('#area-tr-passCd').hide();
	...
	$.ajax({
        url: "${utcp.ctxPath}/admin/testMng/ctgryView.json",
        type: "POST",
        data: {
        	"ctgrySeq" : qstnCtgSeq
        },
        ...
        success: function(r) {
			if(r.result==1){
				$("#qstnCtgSeq").val(r.data.qstnCtgSeq);
				$("#ctgryDepth").val(r.data.ctgryDepth);
				$("#ctgNm").val(r.data.ctgNm);
...
function fn_del(depth){
	...
	$.ajax({
        url: "${utcp.ctxPath}/admin/testMng/ctgryDelete.json",
        type: "POST",
        traditional : true,
        data: {
        	"ctgrySeqs" : ctgrySeqs
...
function fn_excute(){	
	//console.log("fn_excute called");
	//alert("fn_excute called");
	/*
	...
	*/
	...
	if($("#command").val() == "add"){
		$.ajax({
	        url: "${utcp.ctxPath}/admin/testMng/ctgryAdd.json",
	        type: "POST",
	        data: {
	        	"ctgNm" : $("#ctgNm").val(),
	        	"ctgDepth" : $("#ctgryDepth").val(),
		...
		$.ajax({
	        url: "${utcp.ctxPath}/admin/testMng/ctgryUpd.json",
	        type: "POST",
	        data: {
	        	"qstnCtgSeq" : $("#qstnCtgSeq").val(),
	        	"ctgNm" : $("#ctgNm").val(),
...
function fn_orderUp(idx, depth){
	if(idx != 1){
		...
		$.ajax({
	        url: "${utcp.ctxPath}/admin/testMng/ctgryOrder.json",
	        type: "POST",
	        data: {
	        	"selectSeq" : selectSeq,
	        	"targetSeq" : targetSeq
...
<form id="form-view" action="quizCateMng.do">
		...
		<div class="ctgryWrap">
					...
					<ul>
						<c:if test='${cateList1 != null && fn:length(cateList1) > 0}'>
							...
							<c:forEach var="data" items="${cateList1}" varStatus="stat">
								...
								<input type="hidden" id="idx1${idx1}" value="${data.qstnCtgSeq}" />
								<li>
									<span><input type="checkbox" name="ctgrySeqs1" value="${data.qstnCtgSeq}"></span> 
									<span class="tl el " style="${data.qstnCtgSeq eq parentSeq1?'background-color:lightskyblue':''}"> 
									<div class="truncate" style="float:left;" ><a href="javascript:;" title="${data.ctgNm}" onclick="fn_upd('${data.qstnCtgSeq}','1'); return false;">${data.ctgNm}</a></div>
									<a href="quizCateMng.do?parentSeq1=${data.qstnCtgSeq}">[하위보기]</a>
									</span> <span> <c:choose>
			...
			<div class="ctgrytbBth cf">
				<div class="fr tc">
					...
					<button type="button" class="btn04 btn_gray" onclick="fn_del('1');">삭제</button>
				</div>
			</div>
		</div>
		<!--// 교육과정관리 //-->
			...
			<div class="ctgryWrap">
						...
						<ul>
							<c:if test='${cateList2 != null && fn:length(cateList2) > 0}'>
								...
								<c:forEach var="data" items="${cateList2}" varStatus="stat">
									...
									<input type="hidden" id="idx2${idx2}" value="${data.qstnCtgSeq}" />
									<li>
										<span><input type="checkbox" name="ctgrySeqs2" value="${data.qstnCtgSeq}"></span> 
										<span class="tl el" style="${data.qstnCtgSeq eq parentSeq2?'background-color:lightskyblue':''}"> 
										<div class="truncate" style="float:left;" ><a href="javascript:;" title="${data.ctgNm}" onclick="fn_upd('${data.qstnCtgSeq}','2'); return false;">${data.ctgNm}</a></div>
										<a href="quizCateMng.do?parentSeq2=${data.qstnCtgSeq}">[하위보기]</a>
										</span> <span> <c:choose>
												<c:when test='${data.useYn == "Y"}'>
												Yes
											</c:when>
				...
				<div class="ctgrytbBth cf">
					<div class="fr tc">
						...
						<button type="button" class="btn04 btn_gray" onclick="fn_del('2');">삭제</button>
						</c:if>
					</div>
				</div>
			</div>
			...
			<div class="ctgryWrap">
						...
						<ul>
							<c:if test='${cateList3 != null && fn:length(cateList3) > 0}'>
								...
								<c:forEach var="data" items="${cateList3}" varStatus="stat">
									...
									<input type="hidden" id="idx3${idx3}" value="${data.qstnCtgSeq}" />
									<li>
										<span><input type="checkbox" name="ctgrySeqs3" value="${data.qstnCtgSeq}"></span> 
										<span class="tl el"> 
											<a href="javascript:;" title="${data.ctgNm}" onclick="fn_upd('${data.qstnCtgSeq}','3'); return false;">${data.ctgNm}</a>
										</span> 
										<span> 
											<c:choose>
												<c:when test='${data.useYn == "Y"}'>
				...
				<div class="ctgrytbBth cf">
					<div class="fr tc">
						...
						<button type="button" class="btn04 btn_gray" onclick="fn_del('3');">삭제</button>
						</c:if>
					</div>
				</div>
			</div>
...
<div class="remodal" data-remodal-id="ctgryAdd" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc">
		...
		<div class="modal-body">
			<div class="ctgryBox">
				<form id="addFrm" method="post">
					<input type="hidden" id="qstnCtgSeq" />
					...
					<table class="tc tb">
						...
						<tbody>
							<tr>
								<th>명칭</th>
								<td class="tl"><input type="text" name="ctgNm" id="ctgNm" value="" class="ip4"></td>
							</tr>
							<tr>
								<th>사용</th>
							...
							<tr id="area-tr-passCd">
								<th>코드</th>
								<td class="tl"><input type="text" name="passCd" id="passCd" value="" class="ip5"></td>
							</tr>
...
<div class="remodal messagePop1" data-remodal-id="messagePop" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc">
	<div class="modal-content">
		...
		<div class="modal-body">
			<p class="messageTxt">카테고리가 삭제되었습니다.</p>
		</div>
		<div class="modal-footer">
			<div class="tc">
