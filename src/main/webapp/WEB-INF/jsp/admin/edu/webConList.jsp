...
<section class="pd025 pd200 po_re">
	...
	<input type="hidden" id="pageNo" name="pageNo" value="1" />
		...
		<table width="100%" class="tb03 tc mov">
			...
			<tr>
				...
				<span class="dp_b font_14">업로드 : ${utcp.convDateToStr(o.regTime,'yyyy.MM.dd HH:mm') }</span></td>
 				<td class="tl">
					<button class="btn02 btn_blue" onclick="fn_playMov2('http://edu.cdn.gov-ntruss.com/mov/${o.orgCd }/${o.fileRename}');">시청하기</button>
					<c:if test="${sessionScope.sessionUserInfo.orgCd eq o.orgCd}">
		...
		<div class="fr tc">
			<button class="btn01 btn_greenl" onclick="location.href='webConRgs.do'";>등록</button>
		</div>
	</div>
	<jsp:include page="/WEB-INF/jsp/admin/bbs/pageNavi.jsp" />
</section>

<div class="remodal" id="vm-mov2" data-remodal-id="md-mov2" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc" data-remodal-options="closeOnOutsideClick: false">
	<button onclick="fn_md_mov2_close()" class="remodal-close" aria-label="Close"></button>
	<div class="modal-content">
		<div class="modal-body">
			<video id="md-mov2-video" controls="controls" style="width:100%;" />
		</div>
	</div>
</div>
<script>
function fn_md_mov2_close(){
	video = document.getElementById('md-mov2-video');
	if(!video.paused){
		video.pause();
    	video.currentTime = 0;
	}
	$('[data-remodal-id=md-mov2]').remodal().close();
}
</script>
