...
<%
	...
	**********************************************************************************************/
	...
	String resultURL   = domainNm + "/user/ipin_result.do";
...
<head>
	...
	<script>
		function fnLoad()
		{
			...
			vnoform.enc_data.value = "<%= sResponseData %>";
			//parent.opener.parent.document.vnoform.target = "Parent_window";
			vnoform.target = "Parent_window";
			
			// 인증결과 데이터를 최종 수신하는 결과페이지 URL (형식:절대주소, 필수항목:프로토콜) 
			//parent.opener.parent.document.vnoform.action = "http://localhost:8080/user/ipin_result.jsp";
			//parent.opener.parent.document.vnoform.action = "http://localhost:8080/user/ipin_result.do";
			//parent.opener.parent.document.vnoform.action = "%= resultURL %>"; 
			//parent.opener.parent.document.vnoform.submit();
			vnoform.action = "<%= resultURL %>"; 
			vnoform.submit();
